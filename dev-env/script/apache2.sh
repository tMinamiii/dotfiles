...
if [ -e /etc/os-release ]; then
    ...
    brew install -y pcre make openldap
fi
...
if [ -e /etc/os-release ]; then
    if cat /etc/os-release | grep -sq "Ubuntu" || uname -a | grep -sq "Microsoft"; then
        ./configure --quiet --prefix="$APACHE_DIR" --with-crypto --with-included-apr && make && make install
    fi
elif uname -a | grep -sq "Darwin"; then
    ./configure --quiet --prefix="$APACHE_DIR" --with-crypto --with-included-apr GPP=/usr/bin/g++ CC=/usr/bin/cc && make && make install
fi
